                ...
                <thead>
                    <tr style="background:var(--blue-50); border-bottom:1px solid var(--gray-200);">
                        ...
                        <th
                            style="padding:12px 16px; text-align:left; font-weight:700; color:var(--blue-900); font-family:'Plus Jakarta Sans',sans-serif;">
                            Student</th>
                        <th style="padding:12px 16px; text-align:center; font-weight:700; color:var(--blue-600); font-family:'Plus Jakarta Sans',sans-serif;">CBT /20</th>
                        <th style="padding:12px 16px; text-align:center; font-weight:700; color:var(--amber); font-family:'Plus Jakarta Sans',sans-serif;">Test 1 /10</th>
                        <th style="padding:12px 16px; text-align:center; font-weight:700; color:var(--amber); font-family:'Plus Jakarta Sans',sans-serif;">Test 2 /10</th>
                        <th style="padding:12px 16px; text-align:center; font-weight:700; color:var(--blue-600); font-family:'Plus Jakarta Sans',sans-serif;">CA /40</th>
                        <th style="padding:12px 16px; text-align:center; font-weight:700; color:var(--green); font-family:'Plus Jakarta Sans',sans-serif;">Exam /60</th>
                        <th style="padding:12px 16px; text-align:center; font-weight:700; color:var(--blue-900); font-family:'Plus Jakarta Sans',sans-serif;">Total /100</th>
                        <th
                ...
                <tbody>
                    ...
                    <tr style="border-bottom:1px solid var(--gray-100);" id="row_{{ $student->id }}">
                        ...
                        <td style="padding:8px 10px; text-align:center;">
                            <input type="number"
                                name="scores[{{ $student->id }}][test1_score]"
                                value="{{ $score?->test1_score }}"
                                min="0" max="10" step="0.5"
                                placeholder="—"
                                oninput="calcTotal({{ $student->id }})"
                                style="width:60px; padding:6px 8px; border-radius:7px;
                  ...
                  font-size:13.5px; font-family:inherit; color:var(--gray-700);"
                        ...
                        <td style="padding:8px 10px; text-align:center;">
                            <input type="number"
                                name="scores[{{ $student->id }}][test2_score]"
                                value="{{ $score?->test2_score }}"
                                min="0" max="10" step="0.5"
                                placeholder="—"
                                oninput="calcTotal({{ $student->id }})"
                                style="width:60px; padding:6px 8px; border-radius:7px;
                  ...
                  font-size:13.5px; font-family:inherit; color:var(--gray-700);"
                        ...
                        {{-- CA total — read only, computed live --}}
                        <td style="padding:10px 16px; text-align:center;">
                            <span id="ca_{{ $student->id }}"
                                style="font-weight:700; color:var(--blue-600); font-size:14px;">
                        ...
                        <td style="padding:8px 10px; text-align:center;">
                            <input type="number"
                                ...
                                placeholder="—"
                                oninput="calcTotal({{ $student->id }})"
                                style="width:70px; padding:6px 8px; border-radius:7px;
                  border:1.5px solid var(--gray-200); text-align:center;
                  font-size:13.5px; font-family:inherit; color:var(--gray-700);"
...
<script>
    ...
    function calcTotal(studentId) {
        ...
        const test1 = parseFloat(document.querySelector(`input[name="scores[${studentId}][test1_score]"]`)?.value || 0);
        const test2 = parseFloat(document.querySelector(`input[name="scores[${studentId}][test2_score]"]`)?.value || 0);
        const exam = parseFloat(document.querySelector(`input[name="scores[${studentId}][exam_score]"]`)?.value || 0);

        const ca = Math.min(40, cbt + test1 + test2);
        const total = Math.min(100, ca + exam);

        const caEl = document.getElementById(`ca_${studentId}`);
        const totalEl = document.getElementById(`total_${studentId}`);
        const gradeEl = document.getElementById(`grade_${studentId}`);

        if (caEl) caEl.textContent = ca.toFixed(0);
        ...
        gradeEl.className = 'module-badge ' + (['A1'].includes(grade) ? 'badge-green' : ['B2', 'B3'].includes(grade) ? 'badge-blue' : ['C4', 'C5', 'C6'].includes(grade) ? 'badge-blue' : ['D7', 'E8'].includes(grade) ? 'badge-amber' : 'badge-gray');
    }
</script>
@endpush